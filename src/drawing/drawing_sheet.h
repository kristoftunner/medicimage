...
class DrawingSheet
{
  ...
  public:
    ...
    DrawCommandReturn OnMouseButtonPressed(
        const glm::vec2 pos); // assuming only left mouse button can be pressed, BIG TODO:
    ...
    BaseDrawState *GetDrawState()
    {
        return m_drawState.get();
    }
    std::vector<glm::vec2> GetDrawingPoints()
    {
        return std::vector<glm::vec2>{m_firstPoint, m_secondPoint};
    }
  ...
  private:
    ...
    // configs
    static constexpr glm::vec4 s_selectBoxColor{0.23f, 0.55f, 0.70f, 0.5f};
    static constexpr glm::vec4 s_pickPointColor{0.14f, 0.50f, 0.62f, 0.5f};
    static constexpr float s_pickPointBoxSize = 0.02f;
    friend class Entity;
...
{
  public:
    BaseDrawState(DrawingSheet *sheet, const std::string &stateName = "BaseDrawState")
        : m_sheet(sheet), m_stateName(stateName)
    {
    }
    ...
    virtual DrawCommandReturn OnMouseHovered(const glm::vec2 pos)
    {
        return DrawCommandReturn{DrawCommandReturn::State::IN_COMMAND, DrawCommand::DO_NOTHING};
    }
    virtual DrawCommandReturn OnMouseButtonPressed(const glm::vec2 pos)
    {
        return DrawCommandReturn{DrawCommandReturn::State::IN_COMMAND, DrawCommand::DO_NOTHING};
    }
    virtual DrawCommandReturn OnMouseButtonDown(const glm::vec2 pos)
    {
        return DrawCommandReturn{DrawCommandReturn::State::IN_COMMAND, DrawCommand::DO_NOTHING};
    }
    virtual DrawCommandReturn OnMouseButtonReleased(const glm::vec2 pos)
    {
        return DrawCommandReturn{DrawCommandReturn::State::IN_COMMAND, DrawCommand::DO_NOTHING};
    }
    virtual DrawCommandReturn OnTextInput(const std::string &inputText)
    {
        return DrawCommandReturn{DrawCommandReturn::State::IN_COMMAND, DrawCommand::DO_NOTHING};
    }
    virtual DrawCommandReturn OnKeyPressed(KeyCode key)
    {
        return DrawCommandReturn{DrawCommandReturn::State::IN_COMMAND, DrawCommand::DO_NOTHING};
    }
...
{
  public:
    DrawTextInitialState(DrawingSheet *sheet) : BaseDrawState(sheet, "DrawTextInitialState")
    {
        m_sheet->ClearSelectionShapes();
    }
    DrawCommandReturn OnMouseHovered(const glm::vec2 pos) override;
...
{
  public:
    DrawTextState(DrawingSheet *sheet) : BaseDrawState(sheet, "DrawTextState")
    {
        m_sheet->ClearSelectionShapes();
    }
...
{
  public:
    DrawIncrementalLetters(DrawingSheet *sheet) : BaseDrawState(sheet, "DrawIncrementalLetters")
    {
        m_sheet->ClearSelectionShapes();
    }
    DrawCommandReturn OnKeyPressed(KeyCode key) override;
