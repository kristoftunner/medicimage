...
const std::string DrawingSheet::GetDrawCommandName()
{
    ...
    case DrawCommand::DRAW_LINE:
        return "DRAW_LINE";
    case DrawCommand::DRAW_MULTILINE:
        return "DRAW_MULTILINE";
    case DrawCommand::DRAW_TEXT:
        return "DRAW_TEXT";
    case DrawCommand::DRAW_INCREMENTAL_LETTERS:
        return "DRAW_INCREMENTAL_LETTERS";
    case DrawCommand::DRAW_SKIN_TEMPLATE:
        return "DRAW_SKIN_TEMPLATE";
    case DrawCommand::DO_NOTHING:
        return "DO_NOTHING";
    default:
        return "UNKNOWN";
...
std::optional<Entity> DrawingSheet::GetHoveredEntity(const glm::vec2 pos)
{
    ...
    for (auto e : view)
    {
        ...
        std::transform(boundingContour.begin(), boundingContour.end(), std::back_inserter(contour), [](glm::vec2 vec) {
...
bool DrawingSheet::IsUnderSelectArea(Entity entity, glm::vec2 pos)
{
    ...
    std::transform(boundingContour.begin(), boundingContour.end(), std::back_inserter(entityContour),
                   [](glm::vec2 vec) {
                       return cv::Point2f{vec.x, vec.y};
                   });
    ...
    std::vector<cv::Point2f> selectContour{
        cv::Point2f{m_firstPoint.x, m_firstPoint.y}, cv::Point2f{m_firstPoint.x + dx, m_firstPoint.y},
    ...
    if (cv::intersectConvexConvex(selectContour, entityContour, tmp, true) > 0.0)
    {
        ...
        {
            APP_CORE_INFO("Entity:{} selected with bb: tl:{}:{}, tr:{}:{}, br:{}:{}, bl:{}:{}",
                          entity.GetComponent<IDComponent>().ID, boundingContour[0].x, boundingContour[0].y,
                          boundingContour[1].x, boundingContour[1].y, boundingContour[2].x, boundingContour[2].y,
                          boundingContour[3].x, boundingContour[3].y);
            return true;
        }
    }
    return false;
...
static std::unique_ptr<BaseDrawComponentWrapper> CreateComponentWrapper(DrawCommand command, glm::vec2 firstPoint,
                                                                        glm::vec2 secondPoint, glm::vec2 sheetSize,
                                                                        DrawObjectType objectType)
{
    switch (command)
    {
    case DrawCommand::DRAW_CIRCLE: {
        return std::make_unique<CircleComponentWrapper>(
            CircleComponentWrapper::CreateCircle(firstPoint, secondPoint, sheetSize.x / sheetSize.y, objectType));
    }
    case DrawCommand::DRAW_RECTANGLE: {
        return std::make_unique<RectangleComponentWrapper>(
            RectangleComponentWrapper::CreateRectangle(firstPoint, secondPoint, objectType));
    }
    case DrawCommand::DRAW_ARROW: {
        return std::make_unique<ArrowComponentWrapper>(
            ArrowComponentWrapper::CreateArrow(firstPoint, secondPoint, objectType));
    }
    case DrawCommand::DRAW_LINE:
    case DrawCommand::DRAW_MULTILINE: {
        return std::make_unique<LineComponentWrapper>(
            LineComponentWrapper::CreateLine(firstPoint, secondPoint, objectType));
    }
    case DrawCommand::DRAW_SKIN_TEMPLATE: {
        return std::make_unique<SkinTemplateComponentWrapper>(
            SkinTemplateComponentWrapper::CreateSkinTemplate(firstPoint, secondPoint, objectType));
    }
    default: {
...
DrawCommandReturn DrawingTemporaryState::OnMouseButtonDown(const glm::vec2 pos)
{
    ...
    auto wrapper = CreateComponentWrapper(m_sheet->m_currentDrawCommand, m_sheet->m_firstPoint, m_sheet->m_secondPoint,
                                          m_sheet->m_sheetSize, DrawObjectType::TEMPORARY);
...
{ // TODO REFACTOR: see above
    ...
    Entity entity;
    auto wrapper = CreateComponentWrapper(m_sheet->m_currentDrawCommand, m_sheet->m_firstPoint, m_sheet->m_secondPoint,
                                          m_sheet->m_sheetSize, DrawObjectType::PERMANENT);
    wrapper->UpdateShapeAttributes();

    m_sheet->Annotated(); // needed for weird UI feature

    DrawCommandReturn ret;
    if (m_sheet->m_currentDrawCommand ==
        DrawCommand::DRAW_LINE) // line is a special case, because we are drawing multiple NOT connected lines
    {
...
DrawCommandReturn DrawTextInitialState::OnMouseButtonReleased(const glm::vec2 pos)
{
    ...
    if (m_text != "")
    {
        TextComponentWrapper tw(TextComponentWrapper::CreateText(m_sheet->m_firstPoint, m_sheet->m_sheetSize, m_text,
                                                                 s_defaultFontSize, DrawObjectType::TEMPORARY));
...
{
    if (m_text ==
        " ") // first delete the space character, which is there only for showing a blank space when using text input
        m_text = "";
    m_text += inputText;
    TextComponentWrapper tw(TextComponentWrapper::CreateText(m_sheet->m_firstPoint, m_sheet->m_sheetSize, m_text,
                                                             s_defaultFontSize, DrawObjectType::TEMPORARY));
...
{ // exit the command if enter is pressed
    ...
    if (m_text != "")
    {
        ...
        {
            m_text.pop_back();
            TextComponentWrapper tw(TextComponentWrapper::CreateText(
                m_sheet->m_firstPoint, m_sheet->m_sheetSize, m_text, s_defaultFontSize, DrawObjectType::TEMPORARY));
            tw.UpdateShapeAttributes();
            ret = {DrawCommandReturn::State::IN_COMMAND, m_sheet->m_currentDrawCommand};
        }
    }
...
DrawTextState::~DrawTextState()
{
    ...
    {
        TextComponentWrapper tw(TextComponentWrapper::CreateText(m_sheet->m_firstPoint, m_sheet->m_sheetSize, m_text,
                                                                 s_defaultFontSize, DrawObjectType::PERMANENT));
        tw.UpdateShapeAttributes();
        m_sheet->Annotated(); // needed for weird UI feature
    }
}
...
DrawCommandReturn ObjectSelectionState::OnMouseButtonDown(const glm::vec2 pos)
{
    ...
    {                                                                       // baby blue select rectangle
        RectangleComponentWrapper rw(RectangleComponentWrapper::CreateRectangle(
            m_sheet->m_firstPoint, m_sheet->m_secondPoint, DrawObjectType::TEMPORARY));
