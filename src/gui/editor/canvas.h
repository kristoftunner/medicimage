...
struct FPSCounter
{
    ...
    {
        float sum = 0.0f;
        for (auto recordedFps : fpsHistory)
        {
            sum += recordedFps;
        }
...
class Canvas : public wxWindow
{
  public:
    ...
    {
        return m_editor.GetCameraMutex();
    }
    ...
    float GetFPS()
    {
        return m_fpsCounter.GetAverageFPS();
    }
    std::optional<std::string> GetActiveDocumentName()
    {
        return m_editor.GetActiveDocumentName();
    }
    std::optional<float> GetZoomLevel()
    {
        return m_editor.GetZoomLevel();
    }
...
{
  public:
    EditorPanel(wxWindow *parent, wxWindowID id, const wxPoint &pos = wxDefaultPosition,
    ...
    std::mutex &GetCameraMutex()
    {
        return m_canvas->GetCameraMutex();
    }
    Canvas *GetCanvas()
    {
        return m_canvas;
  ...
  private:
    wxStaticText *m_patientIdText = nullptr;
