...
void FileLogger::LogFileOperation(const std::string &filename, FileOperation fileOp)
{
    ...
    json fileEntry = {{"name", filename}, {"date", "2022.02.23"}, {"operation", operation}};
...
ImageDocContainer::ImageDocContainer(const std::string &uuid, const std::filesystem::path &baseFolder)
    : m_uuid(uuid), m_dirPath(baseFolder / uuid)
{
...
void ImageDocContainer::ImageLoad(std::string imageName, const std::filesystem::path &filePath,
...
{
    auto it = std::find_if(m_savedImages.begin(), m_savedImages.end(),
...
void ImageSaverContainer::SelectImageSaver(const std::string &uuid)
{
    ...
    if (uuid != "")
    {
        // first clear the images saved into memory, add the saver if doesnt exist in the container, select and load the
        // images
        if (m_selectedSaver != "")
...
ImageSaverContainer::ImageSaverContainer(const std::filesystem::path &baseFolder) : m_dataFolder(baseFolder)
{
