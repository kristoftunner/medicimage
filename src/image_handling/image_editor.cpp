...
void ImageEditor::DrawCircle(glm::vec2 center, float radius, glm::vec4 color, float thickness, bool filled)
{
    ...
    { // TODO add transparency support with wxGraphicsContext
        ...
        s_dc->SetPen(wxPen(wxColor(static_cast<int>(color.r), static_cast<int>(color.g), static_cast<int>(color.b)),
                           static_cast<int>(thickness)));
        s_dc->SetBrush(
            wxBrush(wxColor(static_cast<int>(color.r), static_cast<int>(color.g), static_cast<int>(color.b), 128)));
        s_dc->DrawCircle(wxPoint{static_cast<int>(center.x), static_cast<int>(center.y)}, static_cast<int>(radius));
    }
    else
    {
        wxPen pen = wxPen(wxColor(static_cast<int>(color.r), static_cast<int>(color.g), static_cast<int>(color.b)),
                          static_cast<int>(thickness));
        wxBrush bush = wxBrush(wxColor(static_cast<int>(color.r), static_cast<int>(color.g), static_cast<int>(color.b)),
...
void ImageEditor::DrawRectangle(glm::vec2 topleft, glm::vec2 bottomright, glm::vec4 color, float thickness, bool filled)
{
    ...
    if ((static_cast<int>(topleft.x) != static_cast<int>(bottomright.x)) &&
        (static_cast<int>(topleft.y) != static_cast<int>(bottomright.y)))
    {
        float alpha = color.a;
        color *= 255.0;
        if (filled)
        {
            ...
            // cv::rectangle(overlay, cv::Point{ static_cast<int>(topleft.x), static_cast<int>(topleft.y) }, cv::Point{
        ...
        else
        {
            wxPen pen = wxPen(wxColor(static_cast<int>(color.r), static_cast<int>(color.g), static_cast<int>(color.b)),
                              static_cast<int>(thickness));
            wxBrush bush =
                wxBrush(wxColor(static_cast<int>(color.r), static_cast<int>(color.g), static_cast<int>(color.b)),
                        wxBRUSHSTYLE_TRANSPARENT);
            s_dc->SetPen(pen);
            s_dc->SetBrush(bush);
            s_dc->DrawRectangle(wxRect(wxPoint{static_cast<int>(topleft.x), static_cast<int>(topleft.y)},
                                       wxPoint{static_cast<int>(bottomright.x), static_cast<int>(bottomright.y)}));
        }
    }
...
void ImageEditor::DrawArrow(glm::vec2 begin, glm::vec2 end, glm::vec4 color, float thickness, double tipLength)
{
    ...
    // Draw the arrowhead
    wxPoint arrowPoints[3] = {{static_cast<int>(end.x), static_cast<int>(end.y)},
                              {static_cast<int>(arrowPoint1.x), static_cast<int>(arrowPoint1.y)},
                              {static_cast<int>(arrowPoint2.x), static_cast<int>(arrowPoint2.y)}};
    wxPen pen = wxPen(wxColor(static_cast<int>(color.r), static_cast<int>(color.g), static_cast<int>(color.b)),
                      static_cast<int>(thickness));
    wxBrush bush = wxBrush(wxColor(static_cast<int>(color.r), static_cast<int>(color.g), static_cast<int>(color.b)),
                           wxBRUSHSTYLE_SOLID);
    s_dc->SetPen(pen);
    s_dc->SetBrush(bush);
    s_dc->DrawLine(wxPoint{static_cast<int>(begin.x), static_cast<int>(begin.y)},
                   wxPoint{static_cast<int>(end.x), static_cast<int>(end.y)});
...
void ImageEditor::DrawLine(glm::vec2 begin, glm::vec2 end, glm::vec4 color, float thickness, double tipLengith)
{
    ...
    s_dc->SetBrush(bush);
    s_dc->DrawLine(wxPoint{static_cast<int>(begin.x), static_cast<int>(begin.y)},
                   wxPoint{static_cast<int>(end.x), static_cast<int>(end.y)}); // TODO: add arrow cuz it is only a line
}
...
void ImageEditor::DrawText(glm::vec2 bottomLeft, const std::string &text, int fontSize, float thickness)
{
    ...
    s_dc->DrawRectangle(wxPoint{static_cast<int>(bottomLeft.x), static_cast<int>(bottomLeft.y)},
                        wxSize{static_cast<int>(w), static_cast<int>(h)});
...
void ImageEditor::DrawSpline(glm::vec2 begin, glm::vec2 middle, glm::vec2 end, int lineCount, glm::vec4 color,
                             float thickness)
{
    glm::vec2 imageSize = {s_image->GetWidth(), s_image->GetHeight()};
    begin *= imageSize;
    ...
    wxPoint splinePoints[3] = {{static_cast<int>(begin.x), static_cast<int>(begin.y)},
                               {static_cast<int>(middle.x), static_cast<int>(middle.y)},
...
std::unique_ptr<Image2D> ImageEditor::ReplaceImageFooter(const std::string &footerText, Image2D &image)
{
    ...
    destDC.Blit(0, 0, static_cast<int>(croppedImageSize.x), static_cast<int>(croppedImageSize.y), &sourceDC,
                s_sideBorder, s_topBorder);
...
std::unique_ptr<Image2D> ImageEditor::RemoveFooter(Image2D &image)
{
    ...
    destDC.Blit(0, 0, static_cast<int>(imageSize.x), static_cast<int>(imageSize.y), &sourceDC, s_sideBorder,
                s_topBorder);
    sourceDC.SelectObject(wxNullBitmap);
    destDC.SelectObject(wxNullBitmap);
